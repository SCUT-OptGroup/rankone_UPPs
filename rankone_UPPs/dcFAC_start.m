%% ***************************************************************
%%
%% dcFAC to the following UBPPs:
%
%   min_{x_i\in{-1,1}^n} \Pi_{i=1}^q(<x_i,Q_ix_i>+<c_i,x_i>+a_i)
%
%%
%% ***************************************************************
function [fobj,xsol,infeas,time,rho] = dcFAC_start(Q,c,a,q,m)
...
if q == 1
    ...
    [xsol,rho] = DCP_QAfactor(C,V,VC,OPTIONS_VMM,rho,sigma);
    ...
    OPTIONS_VMM.Lip = sqrt(p)*maxSnorm;  
    ...
    [xsol,rho] = DCP_PolyQAfac(C1,C2,V,VC1,VC2,OPTIONS_VMM,rho,p,sigma);
